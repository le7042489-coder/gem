...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, **kwargs):
    ...
    if 'llava' in model_name.lower() or "gem" in model_name.lower():        # Load LLaVA model
    ...
    if 'llava' in model_name.lower() or 'gem' in model_name.lower():
        ...
        if not ecg_tower.is_loaded:
            # 【关键修改】传入 None，防止 accelerate 自动对其进行 4-bit 量化
            ecg_tower.load_model(device_map=None)

        # 手动将其移动到 GPU 并转为半精度 (FP16)
        # 这一步确保它既不占用太多显存，又能被 MultiheadAttention 正常计算
